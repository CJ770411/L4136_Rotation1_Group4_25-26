...
#SBATCH --job-name=02b_nanoplot_longread_qc
...
SCRIPT=${PROJECT_ROOT}/scripts/02_reads_qc/02b_longread_qc.sh
if [ ! -f "$SCRIPT" ]; then
...
## Run Nanoplot on merged pass and fail long read Nanoplot data

# Define output directory 
OUTDIR="${PROJECT_ROOT}/results/${SAMPLE}/reads_qc/longread"

# Create output directory
echo "Creating long read output directory for ${SAMPLE}"
mkdir -p "$OUTDIR"
echo "Creating long read output directory for ${SAMPLE}: Finished"
