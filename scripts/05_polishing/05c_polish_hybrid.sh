...
#SBATCH --job-name=05c_polish_hybrid
...
## Date: 22.04.2026
## Description: Script to polish de novo hybrid (Illumina/Nanopore) genome assemblies with raw short and long reads.
...
SCRIPT=${PROJECT_ROOT}/scripts/05_polishing/05c_polish_hybrid.sh
if [ ! -f "$SCRIPT" ]; then
...
printf "\n$(date): Starting ROUND 1 polishing hybrid assembly with long reads\n\n"

# Define output directory 
OUTDIR_ROUND_1="${PROJECT_ROOT}/data/processed/${SAMPLE}/polished/hybrid/round_1"

# Create output directory
printf "\n$(date): Creating ROUND 1 hybrid polishing output directory for ${SAMPLE} in $SLURM_JOB_NAME\n\n"
mkdir -p "$OUTDIR_ROUND_1"
printf "\n$(date): Creating ROUND 1 hybrid polishing output directory for ${SAMPLE} in $SLURM_JOB_NAME: Finished\n\n"

# Define input file (raw assembly to be polished)
RAW_ASSEMBLY="${PROJECT_ROOT}/data/processed/${SAMPLE}/assembly/hybrid/${SAMPLE}_hybrid_assembly.fasta"

# Define output files
ALIGNED_ROUND_1="$OUTDIR_ROUND_1/${SAMPLE}_hybrid_round_1.paf" # Long reads aligned to raw assembly
POLISHED_ROUND_1="$OUTDIR_ROUND_1/${SAMPLE}_hybrid_polished_round_1.fasta" # Round 1 polished assembly

# Use minimap2 to align long reads to raw hybrid assembly
# Outputs PAF file
minimap2 \
...
printf "\n$(date): Finished ROUND 1 polishing hybrid assembly with long reads\n\n"
...
printf "\n$(date): Starting ROUND 2 polishing hybrid assembly with long reads\n\n"

# Define output directory 
OUTDIR_ROUND_2="${PROJECT_ROOT}/data/processed/${SAMPLE}/polished/hybrid/round_2"

# Create output directory
printf "\n$(date): Creating ROUND 2 hybrid polishing output directory for ${SAMPLE} in $SLURM_JOB_NAME\n\n"
mkdir -p "$OUTDIR_ROUND_2"
printf "\n$(date): Creating ROUND 2 hybrid polishing output directory for ${SAMPLE} in $SLURM_JOB_NAME: Finished\n\n"

# Input file is the round 1 polished assembly: "$POLISHED_ROUND_1"


# Define output files
ALIGNED_ROUND_2="$OUTDIR_ROUND_2/${SAMPLE}_hybrid_round_2.paf" # Long reads aligned to round 1 polished assembly
POLISHED_ROUND_2="$OUTDIR_ROUND_2/${SAMPLE}_hybrid_polished_round_2.fasta" # Round 2 polished assembly
...
printf "\n$(date): Finished ROUND 2 polishing hybrid assembly with long reads\n\n"
...
printf "\n$(date): Starting ROUND 3 polishing hybrid assembly with short reads\n\n"

# Define output directory 
OUTDIR_ROUND_3="${PROJECT_ROOT}/data/processed/${SAMPLE}/polished/hybrid/round_3"

# Create output directory
printf "\n$(date): Creating ROUND 3 hybrid polishing output directory for ${SAMPLE} in $SLURM_JOB_NAME\n\n"
mkdir -p "$OUTDIR_ROUND_3"
printf "\n$(date): Creating ROUND 3 hybrid polishing output directory for ${SAMPLE} in $SLURM_JOB_NAME: Finished\n\n"

# Input file is the round 2 polished assembly: "$POLISHED_ROUND_2"

# Define output files
ALIGNED_ROUND_3="$OUTDIR_ROUND_3/${SAMPLE}_hybrid_round_3.rmd.bam" # Short reads aligned to round 2 polished assembly
POLISHED_ROUND_3="$OUTDIR_ROUND_3/${SAMPLE}_hybrid_polished_round_3.fasta" # Round 3 polished assembly

# Index round 2 assembly 
bwa index "$POLISHED_ROUND_2" 
samtools faidx "$POLISHED_ROUND_2"

# Align short reads to round 2 hybrid assembly
bwa mem -M -t $SLURM_CPUS_PER_TASK "$POLISHED_ROUND_2" \
"$READS_SHORT_R1" "$READS_SHORT_R2" \
| samtools view -b \
| samtools sort -o "$OUTDIR_ROUND_3/${SAMPLE}_hybrid_round_3.sort.bam"
...
INPUT="$OUTDIR_ROUND_3/${SAMPLE}_hybrid_round_3.sort.bam" \
OUTPUT="$ALIGNED_ROUND_3" \
METRICS_FILE="$OUTDIR_ROUND_3/${SAMPLE}_hybrid_round_3.rmd.bam.metrics" 

# Index BAM file 
samtools index "$ALIGNED_ROUND_3"

# Remove .sort.bam to clean up
rm "$OUTDIR_ROUND_3/${SAMPLE}_hybrid_round_3.sort.bam"
...
# Re-name default output FASTA to "hybrid_assembly_polished_round_3.fasta"
mv "$OUTDIR_ROUND_3/pilon.fasta" "$POLISHED_ROUND_3" 

# Completing polishing round 3 
printf "\n$(date): Finished ROUND 3 polishing hybrid assembly\n\n"
...
printf "\n$(date): Starting ROUND 4 polishing hybrid assembly with short reads\n\n"

# Define output directory 
OUTDIR_ROUND_4="${PROJECT_ROOT}/data/processed/${SAMPLE}/polished/hybrid/round_4"

# Create output directory
printf "\n$(date): Creating ROUND 4 hybrid polishing output directory for ${SAMPLE} in $SLURM_JOB_NAME\n\n"
mkdir -p "$OUTDIR_ROUND_4"
printf "\n$(date): Creating ROUND 4 hybrid polishing output directory for ${SAMPLE} in $SLURM_JOB_NAME: Finished\n\n"

# Input file is the round 3 polished assembly: "$POLISHED_ROUND_3"

# Define output files
ALIGNED_ROUND_4="$OUTDIR_ROUND_4/${SAMPLE}_hybrid_round_4.rmd.bam" # Short reads aligned to round 3 polished assembly
POLISHED_ROUND_4="$OUTDIR_ROUND_4/${SAMPLE}_hybrid_polished_round_4.fasta" # Round 4 polished assembly

# Index round 3 assembly 
bwa index "$POLISHED_ROUND_3" 
samtools faidx "$POLISHED_ROUND_3"

# Align short reads to round 3 hybrid assembly
bwa mem -M -t $SLURM_CPUS_PER_TASK "$POLISHED_ROUND_3" \
"$READS_SHORT_R1" "$READS_SHORT_R2" \
| samtools view -b \
| samtools sort -o "$OUTDIR_ROUND_4/${SAMPLE}_hybrid_round_4.sort.bam"
...
VALIDATION_STRINGENCY=SILENT \
MAX_FILE_HANDLES_FOR_READ_ENDS_MAP=1000 \
INPUT="$OUTDIR_ROUND_4/${SAMPLE}_hybrid_round_4.sort.bam" \
OUTPUT="$ALIGNED_ROUND_4" \
METRICS_FILE="$OUTDIR_ROUND_4/${SAMPLE}_hybrid_round_4.rmd.bam.metrics" 

# Index BAM file 
samtools index "$ALIGNED_ROUND_4"

# Remove .sort.bam to clean up
rm "$OUTDIR_ROUND_4/${SAMPLE}_hybrid_round_4.sort.bam"
...
# Re-name default output FASTA to "hybrid_assembly_polished_round_4.fasta"
...
printf "\n$(date): Finished ROUND 4 polishing hybrid assembly\n\n"
